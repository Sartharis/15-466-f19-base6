...
Load< SpriteAtlas > order_atlas(LoadTagLate, []() -> SpriteAtlas const * {
	...
	require_plants_in2.insert( std::pair<PlantType const*, int>( cactus_plant, 2 ) );

	std::map< PlantType const*, int > require_plants_in3;
	require_plants_in3.insert( std::pair<PlantType const*, int>( friend_plant, 1 ) );

	std::map< PlantType const*, int > require_plants_in4;
	require_plants_in4.insert( std::pair<PlantType const*, int>( vampire_plant, 1 ) );

	order1 = new OrderType("ORDER", "I need some ferns.",require_plants_in1,300,nullptr);
	order2 = new OrderType("ORDER", "I need cacti for self-defense",require_plants_in2,350,nullptr);
	order3 = new OrderType("ORDER", "My dad has been lonely lately. I need companion carrots.",require_plants_in3,450,nullptr);	
...
void OrderType::draw(glm::uvec2 const &drawable_size, Inventory& inventory) const {
	...
	{// draw text
		...
		draw.draw_text("ORDER for $" + std::to_string( get_bonus_cash() ), glm::vec2( drawable_size.x  - 500.0f, drawable_size.y - 25.0f), 0.6f);
		...
		while( iter != require_plants.end() ) {
			...
			if( require_type )
			{
				...
				draw.draw_text( " - " + require_type->get_name() + ": " + std::to_string( inventory.get_harvest_num( require_type ) ) + "/" + std::to_string(require_num) , 
								glm::vec2( drawable_size.x - 500.0f, drawable_size.y - 75.0f - text_gap ), 0.4f );
