...
struct DrawSprites {
	...
	//Add text to draw:
	void draw_text(std::string const &name, glm::vec2 const &anchor, float scale = 1.0f, glm::u8vec4 const &tint = glm::u8vec4(0xff, 0xff, 0xff, 0xff), float max_width = 10000.f, glm::vec2 *anchor_out = nullptr);

	//Measure text:
	void get_text_extents(std::string const &name, glm::vec2 const &anchor, float scale, glm::vec2 *min, glm::vec2 *max, float max_width = 10000.0f);
